...
import 'package:landregistrationbdios/model_for_api/question_model.dart';
...
class EditAllQuestion extends StatefulWidget {
  EditAllQuestion({Key? key, this.q_Data}) : super(key: key);

  Q_Data? q_Data;

  @override
  State<EditAllQuestion> createState() => _EditAllQuestionState();
}

class _EditAllQuestionState extends State<EditAllQuestion> {
  ...
  void initState() {
    ...
    titleController.text = "${widget.q_Data!.title}";
    describtionController.text = "${widget.q_Data!.description}";
    selectedValue = widget.q_Data!.categoryId;
  }
                                      ...
                                      if (result.statusCode == 200) {
                                        ...
                                        setState(() {
                                          ...
                                          Navigator.pushReplacement(
                                              context,
                                              PageTransition(child: AddQuestionPage(), type: PageTransitionType.leftToRight,duration: Duration(milliseconds: 400)));
                                        });
                                      } else {
                                        setState(() {
                                          isLoading = false;
